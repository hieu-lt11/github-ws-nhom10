...
class LienHeController
{
    ...
    public function store(){
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            if(empty($noi_dung)){
                $errors['noi_dung'] = 'Bạn phải nhập ngày tạo liên hệ' ;
            }
            if(empty($trang_thai)){
    ...
    public function update(){
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            if(empty($noi_dung)){
                $errors['noi_dung'] = 'Bạn phải nhập số điện thoại liên hệ' ;
            }
            if(empty($ngay_tao)){
                $errors['ngay_tao'] = 'Bạn phải nhập ngày tạo liên hệ' ;
            }
            ...
            if (empty($errors)) {
                # nếu không có lỗi thì thêm dữ liệu
                $this->modelLienHe->updateData($id, $ten, $email, $so_dien_thoai,$noi_dung, $ngay_tao,$trang_thai);
                unset($_SESSION['errors']);
                header('Location: ?act=lien-he');
                exit();
            }else{
